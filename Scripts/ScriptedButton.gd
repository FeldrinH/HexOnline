...
func _ready():
	var parse_error = compiled_expression.parse(expression, ["input1", "input2", "input3", "tree", "self"])
	if parse_error:
...
func _pressed():
	compiled_expression.execute([input1_node, input2_node, input3_node, tree_node, self])
